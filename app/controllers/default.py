...
from app.models.tables import User, Coment
...
'''@app.route("/index/main/products/sale/end", defaults={'id': None})
@app.route("/index/main/products/sale/end/<id>")
def sale_end(id):
    form = SaleForm()
    cliente = User.query.filter_by(id).first
    if form.validate_on_submit():
        sales.user_id =
'''
'''
@app.route("/dashboard/view_evento/", defaults={'id': None})
@app.route("/dashboard/view_evento/<id>")
def view_event(id):
    if current_user.is_authenticated:
        evento = Event.query.filter_by(id=int(id)).first()

        if evento != None and evento.user_id == current_user.id:
            participantes = Participant.query.filter_by(event_id=evento.id).all()
            return render_template("view_evento.html", evento = evento, participantes = participantes)
        else:
